...
class DateAxisFeat(pg.AxisItem):
    ...
    def tickStrings(self, values, scale, spacing):
        # strftime   isoformat()
        return [(self.rec_datetime0+dt.timedelta(seconds=value/self.sampling_rate)).strftime("%H:%M:%S") for value in values]
...
class EEGAxis(pg.AxisItem):
    ...
    def tickStrings(self, values, scale, spacing):
        # strftime   isoformat()
        return [(value/self.sampling_rate)-self.eegView_win_size/2 for value in values]
...
class MainWindow(QtGui.QWidget):
    ...
    def initUI(self, params=None):
        ...
        # Usefull params
        self.nb_win_EEG_values = self.params[
            'eegView_win_size'] * self.params['fe']
        ...
        self.connect(subject_combo, QtCore.SIGNAL(
            'activated(QString)'), self.change_subject)
        # EEG Time #TODO
        ...
        axisEEG = EEGAxis(sampling_rate=self.params['fe'], eegView_win_size=self.params[
                          'eegView_win_size'], orientation='bottom')
        # eeg_plot and feat_plot are viewbox
        self.eeg_plot = self.gWin.addPlot(
            row=1, col=0, axisItems={'bottom': axisEEG})
        self.eeg_plot.invertY()
        self.eeg_plot.setMouseEnabled(x=True, y=True)
        ##axis in date
        s_rate = 1/(self.params['featCalc_win_size']
                    * self.params['featCalc_win_overlap'])
        print "sample_rate : ", 1/self.params['featCalc_win_size']
        axisFeat = DateAxisFeat(
            rec_datetime0=dt.datetime.now(), sampling_rate=s_rate, orientation='bottom')
        self.feat_plot = self.gWin.addPlot(
            row=2, col=0, axisItems={'bottom': axisFeat})
        self.feat_plot.setMouseEnabled(x=True, y=True)
        self.main_layout.addWidget(self.gWin, 1, 0)

        # Line EEG : always in the center
        self.vLine_eeg = pg.InfiniteLine(
            pos=np.round(self.nb_win_EEG_values/2), angle=90, movable=False)
        self.eeg_plot.addItem(self.vLine_eeg, ignoreBounds=True)
        # Line feat (movable and update EEG view)
        self.vLine_feat = pg.InfiniteLine(
            pos=self.Feat_Xpos, angle=90, movable=True)
        self.feat_plot.addItem(self.vLine_feat, ignoreBounds=True)
        self.vLine_feat.sigPositionChangeFinished.connect(
    ...
    def load_feat_plots(self):
        ...
        for j in range(nb_feat):
            data = self.feat_df[self.params['features'][j]][self.selected['subject']][
                self.selected['feat_chan']].values.reshape((self.nbFeatSamples))
            min_feat = min(data)
            max_feat = np.percentile(data, 90)
            data = (data - max_feat)/(max_feat - min_feat) + \
                (nb_feat-j) * self.params['feat_offset']
            print np.shape(data)[0]
            self.feat_plot.plot(
                x=range(np.shape(data)[0]), y=data,  pen=(j, nb_feat*1.3))
    ...
    def update_eeg_plots(self):
        ...
        for i, name_chan in enumerate(self.params['channels']):
            eeg_win = self.eeg_df[name_chan][
                x0:x1].values * gain+(nb_chan - i)*chan_offset
    ...
    def loadSubjectdata(self):
        ...
        eeg_Store = pd.HDFStore(join(self.params['data_repo'], self.selected[
                                'subject'], self.selected['subject'] + '.h5'))
        ...
        for feat in self.params['features']:
            ...
            feat_Store = pd.HDFStore(
                join(self.params['data_repo'], feat + '.h5'))  # read_hdf ??
            self.feat_df[feat] = feat_Store.df.dropna()

        # load environment data
        env_raw = join(self.params['data_repo'], self.selected[
                       'subject'], self.selected['subject'] + '.raw')
        env_raw_header = join(self.params['data_repo'], self.selected[
                              'subject'], self.selected['subject'] + '.header')
        d = self.read_header(env_raw_header)

        # (re)set params
        self.nbEEGsamples = np.shape(self.eeg_df['Cz'])[0]
        print "size EEG : ", self.nbEEGsamples
        self.nbFeatSamples = np.shape(
            self.feat_df[self.params['features'][0]])[0]
        print "nb feat samples : ", self.nbFeatSamples
        self.env_sigs = np.fromfile(
            env_raw, dtype=d['dtype'],).reshape(-1, d['nbvoies'])
